...
void pthread_exit(void *retval)
{
	...
	if (pthread_self() == main_thread) {
		/* this could happen if slurmd pthread_create fails several
		 * times and _service_connection is executed without using a
		 * thread */
		sim_lib_printf(0, "pthread_exit for main thread do nothing. Returning\n");
	} else { 
		tid = get_thread_id();
		reps = 0;
		/* A thread could be created and executed so fast pthread_create
		 * wrapper could not set sync tables properly*/
		while ((tid < 0) || (tid > 63)) {
			/* Let's track how often this happens and leave some
			 * time for pthread_create wrapper to work */
			...
			/* Something is wrong. Let's dump some information,
			 * got the global sem then stalls */
			if (reps == 1000) {
				int i = 0;
				printf("Thread %ld: Too many calls to "
					"get_thread_id. Listing current threads...\n",
					pthread_self());
				sem_wait(global_sem);
				while (i < MAX_THREADS) {
		...
		/* Now simulation stuctures are updated. Real pthread_exit
		 * call can be done freely */
		sem_post(global_sem);

		sim_lib_printf(0,
			       "pthread_exit for %d: calling real_pthread_exit\n",
			       tid);
		real_pthread_exit(retval);
	}
	abort();	/* Avoid build warning from a void function "return" */
}
		   ...
		   void *(*start) (void *), void *arg)
{
	...
	/* This wrapper executes the full code with global lock, even call to
	 * real pthread_create */
	sem_wait(global_sem);

	pthread_create_counter[0]++;

	/* First of all, we need a simulation thread id. Under thread creation
	 * bursts could it be no free slot is available. This is what we call
	 * a proto-thread. It should be enough to wait some time till one slot
	 * is available. If the wait takes more than 10 seconds, something is
	...
	/* Calling real pthread_create is needed at this point. We need
	 * pthread_id for linking a simulation thread id with a pthread_id.
	 * It could happen a fast thread trying to use this linking table
	 * before an entry has been created for it, but we are aware of this.
	 * The link is done as one of the last steps because we need to work
	 * for leaving all stable before the thread starts working with
	...
	sim_lib_printf(0,
		       "Dentro de wrapper para pthread_create, getting sleep id %d [%lu]\n",
...
unsigned int sleep(unsigned int seconds)
{
	...
	sim_lib_printf(0,
		       "SIM: sleep_wrapper calling sleep for %d secs (self: %ld - main: %ld)\n",
		       seconds, pthread_self(), main_thread);

	if (seconds == 0) /* Just in case */
		return 0;

	/* There is a issue with synchronization between sim_mgr and a thread
	 * calling sleep. What is needed to be done is to update sleep field
	 * for this thread and wait till such amount of time has been consumed
	 * in simulation time using a explicit semaphore for this thread. It
	 * could happen sim_mgr handling this thread between updating sleep
	 * field and going to sleep with the semaphore. You can not use
	 * global_sem for this two steps, of course, maybe using more sempahores
	...
	if (pthread_self() == main_thread) {
		...
		sim_lib_printf(0,
			       "SIM: sleep_wrapper calling sleep for main thread: %d secs\n",
			       seconds);
		...
		sim_lib_printf(0,
			       "SIM: sleep_wrapper (sval: %d) calling sleep for thread %d [%lu]: %d secs\n",
...
int gettimeofday(struct timeval *tv, struct timezone *tz)
{
	...
	*(current_micro) = *(current_micro) + 100;
	...
	sim_lib_printf(0,
		       "SIM: gettimeofday_wrapper GOT the time data back: %ld - %ld\n",
...
{
	pwd->pw_uid = sim_getuid((char *)name);
	if (pwd->pw_uid == -1) {
		*result = NULL;
...
{
	sim_lib_printf(0,
		      "set_exit_map_array for thread_id: %d, pthread: %ld, thread_exit_array: %16lld\n",
...
int get_thread_id(void)
{
	...
	/* During thread creation could be a problem of thread running but
	 * thread_info_list not created yet */
...
#if 0
{
	...
	sim_lib_printf(0,
		       "get_new_thread_id: [%16llx][%016llx], threads counter= %d\n",
...
/* TODO: It is necessary to use another method for finding out which are the
 * paths to libraries and, maybe, which are the libraries (it seems it is not
 * always the same libraries for different  distributions) */
void __attribute__ ((constructor)) sim_init(void)
{
	...
	if (slurmctl_pid[0] == getpid() || slurmd_pid[0] == getpid()) {
		if (real_pthread_create == NULL) {
			...
			if (real_pthread_create == NULL) {
				printf("Error: no pthread_create function found\n");
				return;
			}
		}
		...
		if (real_pthread_exit == NULL) {
			...
			if (real_pthread_exit == NULL) {
				printf("Error: no pthread_exit function found\n");
				return;
			}
		}
		...
		if (real_pthread_join == NULL) {
			...
			if (real_pthread_join == NULL) {
				printf("Error: no pthread_join function found\n");
				return;
			}
		}
