...
slurm_sprint_job_info(job_info_t *job_ptr, int one_liner, int json_flag)
{
	...
	if (job_ptr->array_job_id) {
		if (json_flag) {
			xstrfmtcat(out, ", \"arrayjobid\":%u",
					job_ptr->array_job_id);
			xstrcat(out, ", \"arraytaskid\":");

			if (job_ptr->array_task_str) {
				xstrcat(out, job_ptr->array_task_str);
			} else {
				xstrfmtcat(out, "%u", job_ptr->array_task_id);
			}
		} else {
			xstrfmtcat(out, "ArrayJobId=%u ArrayTaskId=",
					job_ptr->array_job_id);

			if (job_ptr->array_task_str) {
				xstrfmtcat(out, "%s ", job_ptr->array_task_str);
			} else {
				xstrfmtcat(out, "%u ", job_ptr->array_task_id);
			}
		}
	...
	if (json_flag) {
		xstrfmtcat(out, ", \"gid\":%u, \"group\":\"%s\"",
				job_ptr->group_id,  group_name);
	} else {
		xstrfmtcat(out, "GroupId=%s(%u) ",
